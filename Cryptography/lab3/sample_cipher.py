
from Cryptodome.Util.strxor import strxor

# This file is provided for you to test your implementation, DON'T submit this file.
# ONLY submit `lab3.py`

class Sample_Cipher(object):
    def __init__(self):
        """
            Sample Cipher class that will be passed to your `q1_mitm_cipher` function
        """
        self.KEY_SIZE = 8 #bits
        return
    
    def sub_word(self, word):
        """performs an s-box substitution on the 8-bit input "word"
        
        NOTE: Using this function directly will NOT give you full-credit for question 1
        
        Args:
            word    (bytes/bytearray): bytes/bytearray object of length 8 bits (1 byte).
        
        Return:
            bytes : bytes object of length 8 bits (1 byte)
        """
        sbox = ( # Sample S-Box
            
        )
        return bytes([sbox[b] for b in word])
    
    def inv_sub_word(self, word):
        """ performs an inverse s-box substitution on the 8-bit input "word"
        
        NOTE: Using this function directly will NOT give you full-credit for question 1
        
        Args:
            word    (bytes/bytearray): bytes/bytearray object of length 8 bits (1 byte).
        
        Return:
            bytes : bytes object of length 8 bits (1 byte)
        """
        sbox_inv = ( # Sample S-Box-Inverse
            
        )
        return bytes([sbox_inv[b] for b in word])

    def TOY(self, message, key):
        """preforms a TOY encipher on the 8-bit input "message" given the key "key"
        
        Args:
            message   (str):   hex-encoded string of length 8 bits (1 byte).
            key       (str):   hex-encoded string of length 8 bits (1 byte).
        
        Return:
            str : hex-encoded string of length 8 bits (1 byte)
        """
        return self.__TOY(bytes.fromhex(message), bytes.fromhex(key)).hex()
    
    def __TOY(self, message, key):
        pre_xor     = strxor(message, key)
        sub_word    = self.sub_word(pre_xor)
        post_xor    = strxor(sub_word, key)

        return post_xor
        
    def TOY_inv(self, ciphertext, key):
        """preforms a TOY decipher on the 8-bit input "ciphertext" given the key "key"
        
        Args:
            ciphertext  (str):   hex-encoded string of length 8 bits (1 byte).
            key         (str):   hex-encoded string of length 8 bits (1 byte).
        
        Return:
            str : hex-encoded string of length 8 bits (1 byte)
        """
        return self.__TOY_inv(bytes.fromhex(ciphertext), bytes.fromhex(key)).hex()
    
    def __TOY_inv(self, ciphertext, key):
        post_xor     = strxor(ciphertext, key)
        sub_word    = self.inv_sub_word(post_xor)
        pre_xor    = strxor(sub_word, key)

        return pre_xor

    def TWO_TOY(self, message, key0, key1):
        return self.__TWO_TOY(bytes.fromhex(message), bytes.fromhex(key0), bytes.fromhex(key1)).hex()
    
    def __TWO_TOY(self, message, key0, key1):
        r1_TOY1 = self.__TOY(message, key0)
        mid_val = strxor(r1_TOY1, key1)
        r2_TOY2 = self.__TOY(mid_val, key0)

        return r2_TOY2